...
def main() -> None:
    ...
    header = (f"{'id':<32} {'target':<10} {'flag':<8} "
              f"{'p(target)':>10}   top-{args.top_k}")
    print(header)
    print("-" * (len(header) + 30))
    ...
    for prompt in prompts:
        ...
        if target_id is None:
            ...
            print(f"{pid:<32} {target!r:<10} {'MULTI':<8} {'----':>10}   "
                  f"(target tokenizes to >1 token under {args.model_name})")
            results.append({
                "id": pid, "target": target, "target_literal": target_literal,
                "single_token": False, "p_target": None, "top_k": [],
            })
            continue
        ...
        flag = "PASS" if p_target >= args.min_prob else "FAIL"
        if flag == "FAIL":
            failures.append((pid, target_literal, p_target))

        top_str = ", ".join(f"{repr(t)}={p:.2f}" for t, p in top)
        print(f"{pid:<32} {target_literal!r:<10} {flag:<8} "
              f"{p_target:>10.4f}   {top_str}")

        results.append({
            "id": pid, "target": target, "target_literal": target_literal,
            "single_token": True, "p_target": p_target,
            "top_k": [{"token": t, "p": p} for t, p in top],
        })

    print()
    print(f"Multi-token targets: {len(multi_token)}")
    for pid, t in multi_token:
        print(f"  {pid}: {t!r}")

    print(f"Below threshold p={args.min_prob}: {len(failures)}")
    for pid, lit, p in failures:
        print(f"  {pid}: p({lit!r})={p:.3f}")

    pair_check(prompts, results)
...
def pair_check(prompts: list[dict], results: list[dict]) -> None:
    """If ids end in _pos/_neg, report per-pair both-pass rate."""
    ...
    for stem, pair in by_stem.items():
        ...
        if pos is None or neg is None:
            ...
            continue
        ok_pos = pos.get("single_token") and (pos.get("p_target") or 0) >= 0.2
        ok_neg = neg.get("single_token") and (neg.get("p_target") or 0) >= 0.2
        ...
        print(f"  {stem:<30} {status:<10} "
              f"pos p={pos.get('p_target')!r}  neg p={neg.get('p_target')!r}")
